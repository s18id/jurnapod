...
| **35.2** | Migrate `modules-reservations` to shared Actor | Replace `ReservationGroupActor` and `OutletTableActor`, update service signatures and internal usages. | **4–6h** |
...
| **35.6** | API wrappers and cleanup | Ensure wrappers extract actor from auth context consistently; remove deprecated local actor type aliases/interfaces. | **2–4h** |
...
3. Migrate `modules-reservations` first (contains two actor variants and known prior temporary type from Epic 31-3).

### Phase 3 — Remaining Package Migration
4. Migrate `modules-platform` and `modules-sales`.
5. Migrate `modules-accounting`, `modules-treasury`, and `modules-inventory`.

### Phase 4 — Adapter Cleanup & Hardening
6. Update API wrappers/callers and remove deprecated local actor definitions.
7. Run workspace typecheck and targeted tests for touched modules.
...
- [ ] Deprecated package-local actor interfaces are removed (or explicitly aliased temporarily during migration only).
