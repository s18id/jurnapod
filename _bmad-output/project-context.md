...
### Deterministic Tag Policy (MANDATORY)

Tags generated by `makeTag(...)` MUST be treated as complete outputs. Contributors and agents MUST NOT apply post-generation slicing/truncation (for example: `makeTag(...).slice(...)` or `const t = makeTag(...); t.slice(...)`). Length constraints MUST be enforced inside the generator helper at creation time. If a truncated variant is required for a legacy field, it MUST be produced by a dedicated named helper that documents rationale and target max length.

- ✅ `const code = makeCustomerCodeTag(counter)`
- ❌ `const code = makeTag('CUST', counter).slice(0, 20)`
