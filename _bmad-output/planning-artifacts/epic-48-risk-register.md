...
| R48-004 | P1 | test-infra | Flaky integration behavior may hide real correctness failures | Same suite alternates pass/fail without code changes | Stabilize fixture lifecycle and rerun protocol | @bmad-qa | Sprint 48 | open | 🔄 In progress: Story 48-4 targets fixture lifecycle + 3-consecutive-rerun protocol. Critical suites (fiscal-close, AP reconciliation) show consistent pass in current run (84/84 tests). |
| R48-005 | P1 | process/quality gate | P0/P1 findings might be deferred due to weak closure rules | Sprint marked done with unresolved critical findings | Enforce pre-close no-P0/P1 gate and adversarial GO/NO-GO | @bmad-sm | Sprint 48 | mitigating | ✅ Story 48-5 (CI quality gate enforcement) targets formal enforcement. Sprint closure requires no unresolved P0/P1 per baseline. |
| R48-006 | P2 | api code quality | Touched hotspots retain high `any`/lint debt, reducing confidence in fixes | Type holes in files under active bugfix | Apply touched-scope debt containment only | @bmad-dev | Sprint 48 | mitigating | 🔄 In progress: 180 pre-existing warnings in touched scope. Story 48-6 targets containment in fiscal-close + AP reconciliation touched files. No new `any` introduced by R48-001/R48-002 fixes. |
