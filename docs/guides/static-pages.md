...
- `GET /api/pages/:slug`
  ...
  - Success: `{ success: true, data: { slug, title, content_html, updated_at, published_at } }`.
  - 404 with `{ success: false, error: { code: "NOT_FOUND", message: "Not found" } }` when missing/unpublished.
