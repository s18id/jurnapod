...
import { persistPushBatch, type TransactionPush, type SyncPushResultItem } from './index.js';
...
async function setupTestFixtures(): Promise<TestFixtures> {
  ...
  if (userRows.length === 0) {
    throw new Error(`No users found for company ${config.companyCode}`);
  }
  ...
  return {
    ...
    cashierUserId: Number(userRows[0].id),
  };
}
  ...
  describe('batch splitting with duplicate client_tx_id', () => {
    test('should split batch when duplicate client_tx_id detected in current batch', async () => {
      const transactions: TransactionPush[] = [
        ...
        {
          client_tx_id: 'test-int-tx-split-1', // Duplicate - should trigger split
          company_id: fixtures.testCompanyId,
          outlet_id: fixtures.testOutletId,
          cashier_user_id: fixtures.cashierUserId,
          status: 'COMPLETED',
          service_type: 'TAKEAWAY',
          trx_at: '2024-01-15T16:10:00+07:00',
          items: [{ item_id: 1, qty: 1, price_snapshot: 15000, name_snapshot: 'Test Item' }],
          payments: [{ method: 'CASH', amount: 15000 }],
        },
      ];
