...
-- Add INVOICE_PAYMENT_BANK mapping key to account mappings
-- This separates invoice default payment bank from POS payment method mappings

-- ============================================================
-- Update company_account_mappings check constraint
-- ============================================================
SET @stmt = IF(
  EXISTS (
    SELECT 1
...
ALTER TABLE company_account_mappings
  ADD CONSTRAINT chk_company_account_mappings_key 
  CHECK (mapping_key IN ('AR', 'SALES_REVENUE', 'SALES_TAX', 'SALES_RETURNS', 'INVOICE_PAYMENT_BANK'));

-- ============================================================
-- Update outlet_account_mappings check constraint
-- ============================================================
SET @stmt = IF(
  EXISTS (
    SELECT 1
...
ALTER TABLE outlet_account_mappings
  ...
  CHECK (mapping_key IN ('CASH', 'QRIS', 'CARD', 'SALES_REVENUE', 'SALES_TAX', 'AR', 'INVOICE_PAYMENT_BANK'));
