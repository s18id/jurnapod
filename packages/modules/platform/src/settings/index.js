...
export { SettingValidationError, InvalidSettingsContextError } from "./port.js";
// Cache
...
export function buildPlatformSettingsSeedValues(env) {
    return {
 ...
 */
export async function ensurePlatformSettingsSeeded(db, seedValues, encryptionKey) {
  ...
  `.execute(db);
    ...
    await withTransactionRetry(db, async (trx) => {
        const markerRowsInTx = await sql `
      SELECT setting_key FROM settings_strings WHERE setting_key = ${PLATFORM_SETTINGS_SEED_MARKER_KEY} AND company_id IS NULL AND outlet_id IS NULL LIMIT 1
    `.execute(trx);
 ...
 */
export async function getPlatformSetting(db, key, encryptionKey) {
 ...
 */
export async function setPlatformSetting(db, params, encryptionKey) {
    await withTransactionRetry(db, async (trx) => {
 ...
 */
export async function setBulkPlatformSettings(db, params, encryptionKey) {
    await withTransactionRetry(db, async (trx) => {
 ...
 */
export async function deletePlatformSetting(db, key) {
