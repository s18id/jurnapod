...
import { toUtcIso, CUSTOMER_TYPE } from "@jurnapod/shared";
import { withTransactionRetry } from "@jurnapod/db";
import { CustomerNotFoundError, CustomerCodeConflictError, CustomerValidationError } from "./errors.js";
/**
...
class CustomerServiceImpl {
    ...
    customerRepository;
    accessScopeChecker;
    constructor(db, customerRepository, accessScopeChecker) {
        this.db = db;
        this.customerRepository = customerRepository;
        this.accessScopeChecker = accessScopeChecker;
    }
    ...
    async createCustomer(params) {
        ...
        if (input.type === "BUSINESS" && !input.companyName) {
            throw new CustomerValidationError("Business type customers require a company name");
        }
        return await withTransactionRetry(this.db, async (trx) => {
            ...
            if (existing) {
                throw new CustomerCodeConflictError(`Customer with code ${input.code} already exists`);
            }
