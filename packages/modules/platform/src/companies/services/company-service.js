...
import { MODULE_DEFINITIONS, COMPANY_MODULE_DEFAULTS, ROLE_DEFINITIONS, MODULE_ROLE_DEFAULTS_API, SETTINGS_DEFINITIONS } from "../constants/index.js";
...
import { CompanyNotFoundError, CompanyCodeExistsError, CompanyDeactivatedError, CompanyAlreadyActiveError } from "../interfaces/index.js";
 ...
 */
function buildAuditContext(companyId, actor) {
    return {
 ...
 */
async function ensureDefaultOutlet(db, companyId) {
...
async function upsertRole(db, roleCode, roleName, isGlobal, roleLevel) {
    const existing = await db
        .selectFrom('roles')
        .where('code', '=', roleCode)
        .select(['id'])
...
async function ensureRoles(db) {
    ...
    for (const role of ROLE_DEFINITIONS) {
        roleIds[role.code] = await upsertRole(db, role.code, role.name, role.isGlobal, role.roleLevel);
    }
    return roleIds;
}
async function upsertModule(db, moduleCode, moduleName, moduleDescription) {
    await sql `
...
async function upsertNumberSetting(db, companyId, key, value) {
    await sql `
...
async function upsertBooleanSetting(db, companyId, key, value) {
    await sql `
...
async function ensureDefaultTaxRate(db, companyId) {
    await sql `
    INSERT IGNORE INTO tax_rates (company_id, name, rate_percent, code)
    VALUES (${companyId}, 'VAT', 0.11, 'VAT')
  `.execute(db);
}
async function ensureCompanyModules(db, companyId) {
...
async function ensureCompanyModuleRoles(db, companyId, roleIds) {
    ...
    for (const moduleRoleDefault of MODULE_ROLE_DEFAULTS) {
        ...
        if (!roleId)
            continue;
        if (!validModules.has(moduleRoleDefault.module))
            continue;
        await sql `
...
async function ensureSystemAccounts(db, companyId) {
...
async function bootstrapCompanyDefaults(db, params) {
...
async function ensureCompanyExists(db, companyId, options) {
...
export class CompanyService {
     ...
     */
    async createCompanyBasic(params, db) {
    ...
    async updateCompany(params) {
        return await withTransactionRetry(this.db, async (trx) => {
            ...
            if (Object.keys(updates).length > 0) {
                ...
                await auditService.logUpdate(auditContext, "company", params.companyId, {
     ...
     */
    async deactivateCompany(params) {
        return await withTransactionRetry(this.db, async (trx) => {
            const auditService = new AuditService(trx);
     ...
     */
    async reactivateCompany(params) {
        return await withTransactionRetry(this.db, async (trx) => {
            const auditService = new AuditService(trx);
            const company = await ensureCompanyExists(trx, params.companyId, {
                includeDeleted: true
