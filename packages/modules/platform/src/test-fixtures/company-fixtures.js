 ...
 */
export async function createTestCompanyMinimal(db, options) {
 ...
 */
export async function createTestCompanyWithoutTimezone(db, options) {
    const runId = makeRunId();
    const code = (options?.code ?? `TEST-CO-${runId}`).slice(0, 20).toUpperCase();
    const name = options?.name ?? `Test Company ${runId}`;
...
//# sourceMappingURL=company-fixtures.js.map