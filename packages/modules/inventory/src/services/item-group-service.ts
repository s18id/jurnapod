...
export type { ItemGroup, ItemGroupBulkConflictError };
...
export class ItemGroupServiceImpl implements ItemGroupService {
  async listItemGroups(companyId: number, filters?: { isActive?: boolean }): Promise<ItemGroup[]> {
  ...
  async findItemGroupById(companyId: number, groupId: number): Promise<ItemGroup | null> {
    const db = getInventoryDb();
    ...
    return withTransaction(db, async (trx) => {
      try {
        ...
        if (typeof error === 'object' && error !== null && 'errno' in error && error.errno === 1062) {
          throw new InventoryConflictError("Duplicate item group");
        }
        throw error;
      }
    });
        ...
        if (nextParentId !== before.parent_id) {
          if (nextParentId == null) {
            ...
            if (nextParentId === groupId) {
              throw new InventoryConflictError("Item group parent cannot be itself");
            }
            ...
            if (isDescendant) {
              throw new InventoryConflictError("Item group parent cannot be descendant");
            }
  ...
  async deleteItemGroup(companyId: number, groupId: number, actor?: MutationAuditActor): Promise<boolean> {
    ...
    return withTransaction(db, async (trx) => {
      ...
      if (children.length > 0) {
        throw new InventoryConflictError("Item group has child groups");
      }
...
export const itemGroupService = new ItemGroupServiceImpl();