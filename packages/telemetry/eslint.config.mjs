...
/**
 * ESLint Configuration for @jurnapod/telemetry
 * 
 ...
 * 
 * ADR-0014 Rule 8: Telemetry inside packages must remain framework-agnostic;
 * Hono middleware adapters stay in apps/api.
 * 
...
const config = {
  ...
  ignores: ["dist/**", "node_modules/**", "*.config.*", "**/__tests__/**"],
  languageOptions: {
    parser: tsParser,
    parserOptions: {
        ...
        patterns: [
          {
            ...
            message:
              "[ADR-0014] packages/** must never import from apps/** (or API aliases/paths). Telemetry must remain framework-agnostic.",
          },
        ],
        paths: [
          {
