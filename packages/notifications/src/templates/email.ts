...
/**
 * Email template rendering functions for password reset, user invite, and email verification.
 * These templates produce bilingual (Indonesian/English) email content.
 */
...
export function buildPasswordResetEmail(params: EmailTemplateParams): BuiltEmail {
...
export function buildUserInviteEmail(params: EmailTemplateParams): BuiltEmail {
  const { userName, companyName, actionUrl, expiryHours } = params;
...
export function buildVerifyEmail(params: EmailTemplateParams): BuiltEmail {
  const { userName, companyName, actionUrl, expiryHours } = params;
