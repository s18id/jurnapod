...
const ENV_KEY = "CRON_EMAIL_OUTBOX_SECRET";
const ENV_KEY_PATTERN = /^\s*(?:export\s+)?CRON_EMAIL_OUTBOX_SECRET\s*=/;
const AUTHORITATIVE_ENV_KEY_PATTERN = /^CRON_EMAIL_OUTBOX_SECRET=/;
...
async function main() {
  ...
  await writeSecretEvent({
    event: "CRON_EMAIL_SECRET_REGENERATE",
    status: "SUCCESS",
    targetPath,
    replaced,
  ...
  process.stdout.write("Update cron configuration with the new secret.\n");
