...
const ENV_KEY = "PLATFORM_SETTINGS_ENCRYPTION_KEY";
const ENV_KEY_PATTERN = /^\s*(?:export\s+)?PLATFORM_SETTINGS_ENCRYPTION_KEY\s*=/;
const AUTHORITATIVE_ENV_KEY_PATTERN = /^PLATFORM_SETTINGS_ENCRYPTION_KEY=/;
...
async function main() {
  ...
  await writeSecretEvent({
    event: "PLATFORM_ENCRYPTION_KEY_REGENERATE",
    status: "SUCCESS",
    targetPath,
    replaced,
  ...
  process.stdout.write("WARNING: Regenerating this key will make existing encrypted settings unreadable.\n");
  process.stdout.write("Restart API services to apply the new encryption key.\n");
