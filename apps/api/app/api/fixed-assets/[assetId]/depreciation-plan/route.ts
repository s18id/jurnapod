...
export const GET = withAuth(
  async (request, auth) => {
    try {
      ...
      console.error("GET /fixed-assets/:id/depreciation-plan failed", error);
...
export const POST = withAuth(
  async (request, auth) => {
    try {
      ...
      console.error("POST /fixed-assets/:id/depreciation-plan failed", error);
      return Response.json(INTERNAL_SERVER_ERROR_RESPONSE, { status: 500 });
    }
  },
  [requireRole(["OWNER", "ADMIN", "ACCOUNTANT"])]
...
export const PATCH = withAuth(
  async (request, auth) => {
    try {
      ...
      console.error("PATCH /fixed-assets/:id/depreciation-plan failed", error);
      return Response.json(INTERNAL_SERVER_ERROR_RESPONSE, { status: 500 });
    }
  },
  [requireRole(["OWNER", "ADMIN", "ACCOUNTANT"])]
