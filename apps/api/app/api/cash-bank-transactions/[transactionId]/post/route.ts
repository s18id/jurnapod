...
export const POST = withAuth(
  async (request, auth) => {
    try {
      ...
      const posted = await postCashBankTransaction(auth.companyId, transactionId, { userId: auth.userId });
      return successResponse(posted);
    } catch (error) {
      ...
      return errorResponse("INTERNAL_SERVER_ERROR", "Cash/bank post failed", 500);
    }
  },
  [
    requireAccess({
