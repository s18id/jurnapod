...
export const GET = withAuth(
  async (request, auth) => {
    try {
      ...
      return successResponse(accountTypes);
    } catch (error) {
      if (error instanceof ZodError) {
        return errorResponse("INVALID_REQUEST", "Invalid request parameters", 400);
      }

      console.error("GET /api/accounts/types failed", error);
...
export const POST = withAuth(
  async (request, auth) => {
    try {
      ...
      return successResponse(accountType, 201);
    } catch (error) {
      ...
      console.error("POST /api/accounts/types failed", error);
      return errorResponse("INTERNAL_ERROR", "Internal server error", 500);
    }
  },
