...
const outletGuardSchema = SalesCreditNoteCreateRequestSchema.pick({
  outlet_id: true
});
...
export const GET = withAuth(
  async (request, auth) => {
    try {
      ...
      const parsed = SalesCreditNoteListQuerySchema.parse({
      ...
      const result = await listCreditNotes(auth.companyId, {
        outletIds,
...
export const POST = withAuth(
  async (request, auth) => {
    try {
      ...
      const input = SalesCreditNoteCreateRequestSchema.parse(payload);
      const creditNote = await createCreditNote(auth.companyId, input, {
        userId: auth.userId
      });
      ...
      if (error instanceof DatabaseReferenceError) {
        ...
        return errorResponse("NOT_FOUND", "Invoice not found or not posted", 404);
      }
