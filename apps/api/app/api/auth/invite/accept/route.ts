...
export async function POST(request: Request) {
  ...
  try {
    ...
    let tokenInfo;
    try {
      tokenInfo = await validateAndConsumeToken(connection, validated.token, "INVITE");
    } catch (error) {
    ...
    const passwordHash = await hashPassword(validated.password, policy);
    ...
    try {
      ...
      await auditService.logAction(
        ...
        tokenInfo.userId,
        "UPDATE",
        { action: "invite_accepted", email: tokenInfo.email }
      );
    } catch (auditError) {
      console.error("Failed to write audit log for invite accept", auditError);
      // Don't fail the request if audit fails
    }

    return successResponse({ message: "Account activated successfully" });
  } catch (error) {
    if (connection) {
      try {
        await connection.rollback();
    ...
    return errorResponse("INTERNAL_SERVER_ERROR", "Failed to accept invitation", 500);
  }
}
