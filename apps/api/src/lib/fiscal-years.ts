...
import { APReconciliationError } from "@/lib/purchasing/ap-reconciliation.js";
...
async function triggerAutoSnapshotForFiscalYearClose(input: {
  ...
  reasonPrefix: string;
}): Promise<AutoSnapshotWarning | null> {
  try {
    ...
    if (!Number.isSafeInteger(snapshotActorUserId) || snapshotActorUserId <= 0) {
      ...
      if (!fallbackActor) {
        return {
          code: "AUTO_SNAPSHOT_USER_NOT_FOUND",
          reason: "AUTO_SNAPSHOT_USER_NOT_FOUND",
          message: `Auto AP reconciliation snapshot could not resolve actor user for company ${input.companyId}`,
          blocking: false,
        };
      }
    ...
    if (!fiscalYear?.end_date) {
      return {
        code: "AUTO_SNAPSHOT_NO_END_DATE",
        reason: "AUTO_SNAPSHOT_NO_END_DATE",
        message: `Fiscal year ${input.fiscalYearId} has no end_date; cannot determine as_of_date for auto snapshot`,
        blocking: false,
      };
    }
    ...
    console.warn("Auto AP reconciliation snapshot after fiscal year close failed (non-blocking)", {
      companyId: input.companyId,
    ...
    if (autoSnapshotError instanceof APReconciliationError) {
      const errorCode = autoSnapshotError.code;
      if (errorCode === "AP_RECONCILIATION_SETTINGS_REQUIRED") {
        return {
          code: "AUTO_SNAPSHOT_NOT_CREATED",
          reason: "AP_RECONCILIATION_SETTINGS_REQUIRED",
          message: "Auto AP reconciliation snapshot was not created. Configure AP account_ids via PUT /api/purchasing/reports/ap-reconciliation/settings.",
          blocking: false,
        };
      }
    }
    ...
    return {
      ...
      reason: (autoSnapshotError instanceof APReconciliationError) ? autoSnapshotError.code : "AUTO_SNAPSHOT_FAILED",
      message: `Auto AP reconciliation snapshot failed after fiscal year close: ${autoSnapshotError instanceof Error ? autoSnapshotError.message : String(autoSnapshotError)}. Snapshot can be created manually.`,
      blocking: false,
    };
  }
}
...
export interface ApproveFiscalYearCloseResult {
  ...
  snapshotWarning?: AutoSnapshotWarning;
}
...
export async function approveFiscalYearClose(
  ...
  if (shouldAttemptAutoSnapshot) {
    snapshotWarning = await triggerAutoSnapshotForFiscalYearClose({
      db: db as KyselySchema,
      companyId,
      fiscalYearId,
