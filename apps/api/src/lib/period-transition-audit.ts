...
import type { AuditDbClient } from "@jurnapod/modules-platform";
 ...
 */
export async function logPeriodTransition(
  ...
  // Use logAction for custom action with entity_type as period_transition
  // Entity ID format: {fiscalYearId}-{periodNumber}
  // Note: entity_type is stored as string in DB even though the type signature uses the enum
  await (auditService.logAction as any)(
    context,
    "period_transition",
 ...
 */
export async function queryPeriodTransitionAudits(
 ...
 */
export async function getPeriodTransitionAuditById(
  companyId: number,
  auditId: number
