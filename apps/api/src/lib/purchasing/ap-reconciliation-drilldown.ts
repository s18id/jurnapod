...
/**
 * AP Reconciliation Drilldown Service (Story 47.2 B2A)
 *
 * Provides detailed drill-down into AP vs GL reconciliation variances:
 * - GL detail: all journal lines for AP control accounts within cutoff
 * - AP detail: all open AP transactions (invoices, credits, payments)
 * - Drilldown: variance attribution into deterministic categories
 * - CSV export: full variance data export
 *
 * Key behaviors preserved from B1 (getAPReconciliationSummary):
 * - Fail-closed: returns error if no valid account set resolved
 * - FX conversion: base = original * exchange_rate using scaled bigint math
 * - Timezone handling: outlet -> company, NO UTC fallback
 * - Cutoff consistency: same as summary endpoint
 *
 * Attribution category precedence (deterministic):
 * 1. currency_rounding_differences (<= tolerance)
 * 2. posting_errors (amount mismatch > tolerance)
 * 3. timing_differences (posted but not yet matched by date)
 * 4. missing_transactions (no corresponding entry found)
 */
...
// =============================================================================
// Rounding Tolerance (configurable, default 0.01 in base currency)
// =============================================================================
 ...
 * Uses pagination with cursor/limit (default 100, max 500).
 * Cutoff: journal_batches.posted_at <= as_of_date UTC end boundary.
 */
 ...
 * Returns open amounts in base currency.
 * Cutoff: invoice_date/credit_date/payment_date <= as_of_date.
 */
export async function getAPDetail(
  ...
  // Total count (for pagination metadata) without loading all rows into memory
  ...
  const dataItems = hasMore ? paginatedItems.slice(0, limit) : paginatedItems;
...
async function getAPDetailTotalOpenBase(
  ...
  // Canonical AP subledger total mirrors summary logic (open invoice base only,
  // where payments/credits reduce invoice open via paid_base/credited_base).
...
export function buildDrilldownAttribution(
  ...
  // Decision resolution: classify source-less/unmappable GL lines as missing_transactions
  // to align with AC1 (“GL entry with no AP source”).
  for (const gl of glLines) {
    const normalizedType = normalizePurchasingDocType(gl.sourceType);
...
// =============================================================================
// Main Drilldown Entry Point
// =============================================================================

/**
 * Get full AP reconciliation drilldown.
 * Combines GL detail, AP detail, and attribution logic.
 */
export async function getAPReconciliationDrilldown(
  ...
  const [glResult, apResult] = await Promise.all([
    glDone
      ...
      : getGLDetail(companyId, settings.accountIds, asOfDate, timezone, cursorState.gl, limit),
    apDone
      ...
      : getAPDetail(companyId, asOfDate, cursorState.ap, limit),
  ]);
 ...
 * Columns: category, id, ap_transaction_type, ap_transaction_ref, ap_date,
 *          ap_amount_original, ap_amount_base, ap_currency,
 *          gl_journal_line_id, gl_journal_number, gl_effective_date,
 *          gl_description, gl_amount, gl_debit_credit,
 *          matched, difference, suggested_action
 */
