...
/**
 * AP Aging Report Domain Service
 *
 * Read-only purchasing report service.
 * Computes open AP balances by supplier with aging buckets and detail drill-down.
 */
...
type AgingBucketKey = "current" | "due_1_30" | "due_31_60" | "due_61_90" | "due_over_90";
...
export async function getAPAgingSummary(
  ...
  const rows = await getRawOpenInvoiceRows(db, companyId);
...
export async function getAPAgingSupplierDetail(
  ...
  const db = getDb() as KyselySchema;
  const rows = await getRawOpenInvoiceRows(db, companyId, supplierId);
  ...
  const invoices: APAgingDetailRow[] = [];
