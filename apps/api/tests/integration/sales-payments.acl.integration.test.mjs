...
test(
  "sales payments ACL: tenant isolation and role-based post access",
  ...
  async () => {
    ...
    try {
      ...
      await db.execute(
        ...
        [accountantUserId, accountantRoleId, ownerOutletId]
      );
      await db.execute(
        `INSERT INTO user_outlets (user_id, outlet_id) VALUES (?, ?)`,
        [accountantUserId, ownerOutletId]
      );
      ...
      await db.execute(
        `INSERT INTO user_role_assignments (user_id, role_id, outlet_id) VALUES (?, ?, ?)`,
        [cashierUserId, cashierRoleId, ownerOutletId]
      );
      await db.execute(
        `INSERT INTO user_outlets (user_id, outlet_id) VALUES (?, ?)`,
        [cashierUserId, ownerOutletId]
      );
