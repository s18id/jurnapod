...
test(
  "import integration: should reject requests without authentication (401)",
...
test(
  "import integration: should reject requests with invalid token (401)",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should upload and parse CSV file successfully",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should reject files exceeding 50MB limit (400)",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should reject invalid file types (400)",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should validate mapped data successfully",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should return validation errors for invalid data",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should reject invalid entity type (400)",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should reject expired/missing uploadId (404)",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should create new items successfully",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
${testSkuPrefix}-001,Test Item 1,INVENTORY,${runId}0001,true
${testSkuPrefix}-002,Test Item 2,SERVICE,${runId}0002,true`;

    try {
      // Clean up any existing items with this SKU prefix first
      await db.execute(`DELETE FROM items WHERE company_id = ? AND sku LIKE ?`, [companyId, `${testSkuPrefix}-%`]);
      ...
      const [items] = await db.execute(
        `SELECT id, sku, name, item_type FROM items WHERE company_id = ? AND sku LIKE ?`,
        [companyId, `${testSkuPrefix}-%`]
      );
...
test(
  "import integration: should handle duplicate SKU within same company correctly",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should handle partial failures gracefully",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should download CSV template successfully",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should have correct headers for template",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should return proper content-type for template",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should upload and parse price CSV successfully",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should validate price data successfully",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: prices template should have correct columns",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
test(
  "import integration: should clean up session after apply",
  { timeout: TEST_TIMEOUT_MS, concurrency: false },
  async () => {
    if (typeof loadEnvFile === "function" && existsSync(ENV_PATH)) {
      loadEnvFile(ENV_PATH);
...
${runId}-SESSION-001,Session Test Item,INVENTORY,${runId}0001,true`;
