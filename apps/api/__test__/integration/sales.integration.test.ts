...
describe("Sales Integration Tests", () => {
  ...
  try {
    ...
    await runSubtest("@slow Invoice draft create/update + post creates journal batch", { timeout: 30000 }, async () => {
    ...
    await runSubtest("@slow Invoice due_date defaults to Net 30 and supports common due_term options", { timeout: 30000 }, async () => {
    ...
    await runSubtest("@slow Invoice due_date explicit value overrides due_term and PATCH due_term recalculates", { timeout: 30000 }, async () => {
    ...
    await runSubtest("@slow Payment draft create + post creates journal batch", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
    ...
    await runSubtest("@slow Payment post decreases invoice outstanding correctly", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
      const paymentNo = `TEST-PAY-${randomUUID().slice(0, 8)}`;
      testInvoiceNos.push(invoiceNo);
      testPaymentNos.push(paymentNo);
    ...
    await runSubtest("@slow Partial payment transitions UNPAID -> PARTIAL -> PAID", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
    ...
    await runSubtest("@slow Overpayment fails when variance gain account not configured", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
      const paymentNo = `TEST-PAY-${randomUUID().slice(0, 8)}`;
      testInvoiceNos.push(invoiceNo);
      testPaymentNos.push(paymentNo);
    ...
    await runSubtest("@slow Partial payment without flag stays PARTIAL", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
      const paymentNo = `TEST-PAY-${randomUUID().slice(0, 8)}`;
      testInvoiceNos.push(invoiceNo);
      testPaymentNos.push(paymentNo);
    ...
    await runSubtest("@slow PATCH split payment with actual_amount_idr mismatch rejects with 400", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
      const paymentNo = `TEST-PAY-${randomUUID().slice(0, 8)}`;
      testInvoiceNos.push(invoiceNo);
      testPaymentNos.push(paymentNo);
    ...
    await runSubtest("@slow PATCH split payment with actual_amount_idr matching split total persists consistent payment amount", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
      const paymentNo = `TEST-PAY-${randomUUID().slice(0, 8)}`;
      testInvoiceNos.push(invoiceNo);
      testPaymentNos.push(paymentNo);
    ...
    await runSubtest("@slow Payment post rejects malformed JSON body with 400", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
      const paymentNo = `TEST-PAY-${randomUUID().slice(0, 8)}`;
      testInvoiceNos.push(invoiceNo);
      testPaymentNos.push(paymentNo);
    ...
    await runSubtest("@slow Manual shortfall settlement closes invoice with loss", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
      const paymentNo = `TEST-PAY-${randomUUID().slice(0, 8)}`;
      testInvoiceNos.push(invoiceNo);
      testPaymentNos.push(paymentNo);
    ...
    await runSubtest("@slow Duplicate post calls for payments are idempotent", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
      const paymentNo = `TEST-PAY-${randomUUID().slice(0, 8)}`;
      testInvoiceNos.push(invoiceNo);
      testPaymentNos.push(paymentNo);
    ...
    await runSubtest("@slow Concurrent post calls for payments create one journal batch", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
      const paymentNo = `TEST-PAY-${randomUUID().slice(0, 8)}`;
      testInvoiceNos.push(invoiceNo);
      testPaymentNos.push(paymentNo);
    ...
    await runSubtest("@slow Duplicate post calls are idempotent", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
      testInvoiceNos.push(invoiceNo);
    ...
    await runSubtest("@slow Invoice create is idempotent with client_ref", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
    ...
    await runSubtest("@slow Payment create is idempotent with client_ref", { timeout: 30000 }, async () => {
      const invoiceNo = `TEST-INV-${randomUUID().slice(0, 8)}`;
      const paymentNo = `TEST-PAY-${randomUUID().slice(0, 8)}`;
      const clientRef = randomUUID();
      testInvoiceNos.push(invoiceNo);
    ...
    await runSubtest("@slow Manual journal create is idempotent with client_ref", { timeout: 30000 }, async () => {
      const clientRef = randomUUID();
    ...
    await runSubtest("@slow Credit note create + post creates balanced journal batch", { timeout: 30000 }, async () => {
    ...
    await runSubtest("@slow Posted credit note void creates reversal journal", { timeout: 30000 }, async () => {
    ...
    await runSubtest("@slow Over-credit rejected with no journal side effects", { timeout: 30000 }, async () => {
    ...
    await runSubtest("@slow Credit note create is idempotent with client_ref", { timeout: 30000 }, async () => {
    ...
    await runSubtest("@slow Credit note create without client_ref is non-idempotent", { timeout: 30000 }, async () => {
...
}); // end describe("Sales Integration Tests")
