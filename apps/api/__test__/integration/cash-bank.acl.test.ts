...
import { setupIntegrationTests, loginUser, readEnv, TEST_TIMEOUT_MS } from "../../tests/integration/integration-harness.js";
...
test("@slow cash-bank ACL: role-based access control", { timeout: TEST_TIMEOUT_MS, concurrent: false }, async () => {
  ...
  try {
    ...
    await runSubtest("OWNER can create draft", async () => {
    ...
    await runSubtest("ACCOUNTANT can create draft", async () => {
    ...
    await runSubtest("CASHIER cannot create draft", async () => {
    ...
    await runSubtest("ACCOUNTANT can post draft", async () => {
    ...
    await runSubtest("ACCOUNTANT can void posted", async () => {
      if (createdTxIds.length < 2) return;
      const txId = createdTxIds[1];
    ...
    await runSubtest("CASHIER cannot post", async () => {
    ...
    await runSubtest("CASHIER cannot void", async () => {
      if (createdTxIds.length < 1) return;
      const txId = createdTxIds[0];
    ...
    await runSubtest("outlet scoping: ACCOUNTANT can query assigned outlet A", async () => {
    ...
    await runSubtest("outlet scoping: ACCOUNTANT cannot query unassigned outlet B", async () => {
    ...
    await runSubtest("outlet scoping: ACCOUNTANT can create with assigned outlet", async () => {
      const res = await apiRequest(baseUrl, accountantToken, "/api/cash-bank-transactions", {
        method: "POST",
    ...
    await runSubtest("outlet scoping: ACCOUNTANT cannot create with non-assigned outlet", async () => {
      const res = await apiRequest(baseUrl, accountantToken, "/api/cash-bank-transactions", {
        method: "POST",
    ...
    // ========================================
    await runSubtest("tenant isolation: Company B cannot see Company A transactions in list", async () => {
      if (createdTxIds.length < 1) return;
    ...
    await runSubtest("tenant isolation: Company B cannot post Company A transaction", async () => {
      if (createdTxIds.length < 1) return;
      const txId = createdTxIds[0];
    ...
    await runSubtest("tenant isolation: Company B cannot void Company A transaction", async () => {
      if (createdTxIds.length < 1) return;
      const txId = createdTxIds[0];
      ...
      await db.execute(
        `INSERT INTO user_role_assignments (user_id, role_id, outlet_id, company_id)
         ...
         WHERE r.code = 'OWNER'
           AND o.id = ?
         LIMIT 1`,
        [userB2Id, outletB2Id, outletB2Id]
      );
