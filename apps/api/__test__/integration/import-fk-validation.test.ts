...
test(
  "@slow import FK validation: should pass validation when item_group_id exists",
...
test(
  "@slow import FK validation: should fail validation when item_group_id does not exist",
  { timeout: TEST_TIMEOUT_MS, concurrent: false },
  async () => {
    const loadEnvFile = process.loadEnvFile;
    if (typeof loadEnvFile === "function") {
...
test(
  "@slow import FK validation: should handle mixed valid and invalid FKs correctly",
  { timeout: TEST_TIMEOUT_MS, concurrent: false },
  async () => {
    const loadEnvFile = process.loadEnvFile;
    if (typeof loadEnvFile === "function") {
...
test(
  ...
  async () => {
    ...
    try {
      ...
      // Assign user to outlet via role assignment
      await db.execute(
        `INSERT INTO user_role_assignments (user_id, role_id, outlet_id, company_id)
         ...
         WHERE r.code = 'CASHIER'
           AND o.id = ?
         LIMIT 1`,
        [companyBUserId, companyBOutletId, companyBOutletId]
      );
