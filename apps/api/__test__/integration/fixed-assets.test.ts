...
test(
  "@slow fixed-assets integration: unauthenticated requests return 401",
...
test(
  "@slow fixed-assets integration: full CRUD lifecycle — category and asset",
  { timeout: TEST_TIMEOUT_MS },
  async () => {
    const baseUrl = testContext.baseUrl;
...
test(
  "@slow fixed-assets integration: invalid request bodies return 400",
  { timeout: TEST_TIMEOUT_MS },
  async () => {
    const baseUrl = testContext.baseUrl;
    const token = await loginOwner(baseUrl, COMPANY_CODE, OWNER_EMAIL, OWNER_PASSWORD);
...
test(
  "@slow fixed-assets integration: non-existent resources return 404",
  { timeout: TEST_TIMEOUT_MS },
  async () => {
    const baseUrl = testContext.baseUrl;
    const token = await loginOwner(baseUrl, COMPANY_CODE, OWNER_EMAIL, OWNER_PASSWORD);
