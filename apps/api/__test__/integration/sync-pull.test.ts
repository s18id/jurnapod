...
describe("Sync Pull Routes", { concurrent: false }, () => {
  ...
  beforeAll(async () => {
    ...
    const mainOutletRows = await sql<{ id: number }>`
      SELECT id FROM outlets WHERE company_id = ${testCompanyId} AND code = ${TEST_OUTLET_CODE} LIMIT 1
    `.execute(db);
    assert.ok(mainOutletRows.rows.length > 0, `Outlet ${TEST_OUTLET_CODE} not found`);
    testOutletId = Number(mainOutletRows.rows[0].id);
