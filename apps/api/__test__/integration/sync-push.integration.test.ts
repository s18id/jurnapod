...
test(
  "@slow sync push integration: test headers are ignored without explicit test-hook mode",
    ...
    try {
      ...
      await db.execute(
        `INSERT INTO user_role_assignments (user_id, role_id, outlet_id, company_id)
         VALUES (?, ?, NULL, ?)`,
        [adminUserId, Number(adminRoleId), Number(owner.company_id)]
      );

      await db.execute(
        `INSERT INTO user_role_assignments (user_id, outlet_id, role_id, company_id)
         VALUES (?, ?, ?, ?)`,
        [adminUserId, Number(owner.outlet_id), Number(adminRoleId), Number(owner.company_id)]
      );
      ...
      await db.execute(
        `INSERT INTO user_role_assignments (user_id, role_id, outlet_id, company_id)
         ...
         WHERE r.code = 'CASHIER'
           AND o.id = ?
         LIMIT 1`,
        [foreignUserId, foreignOutletId, foreignOutletId]
      );
      ...
      if (cashierRoleId) {
        await db.execute(
          `INSERT INTO user_role_assignments (user_id, role_id, outlet_id, company_id)
           ...
           WHERE o.id = ?
           LIMIT 1`,
          [foreignUserId, cashierRoleId, foreignOutletId, foreignOutletId]
        );
      }
