...
describe('pages-unpublish', { timeout: 30000 }, () => {
  beforeAll(async () => {
    ...
    const sharedAdminUser = await createTestUser(sharedContext.companyId, {
      email: `settings-unpublish-shared-${Date.now()}@example.com`,
      password: testPassword
    });
    const sharedRole = await createTestRole(baseUrl, cashierToken, 'Settings Unpublisher Shared');
    ...
    const sharedLimitedUser = await createTestUser(sharedContext.companyId, {
      email: `settings-unpublish-limited-${Date.now()}@example.com`,
      password: testPassword
    });
