...
function startApiServer(port) {
  ...
  const child = spawn(process.execPath, ["--import", "tsx", serverScriptPath], {
    cwd: apiRoot,
...
async function listAccounts(baseUrl, accessToken, companyId) {
  ...
  if (!data || !Array.isArray(data.data)) {
    console.info(`Unexpected accounts response: ${JSON.stringify(data)}`);
...
async function createAccount(baseUrl, accessToken, payload) {
...
async function ensureDepreciationAccounts(baseUrl, accessToken, companyId) {
  let accounts = await listAccounts(baseUrl, accessToken, companyId);
  ...
  if (accounts.length < 2) {
    ...
    accounts = await listAccounts(baseUrl, accessToken, companyId);
  }

  if (accounts.length < 2) {
    console.info(`Insufficient accounts found: ${accounts.length}`);
...
async function ensureFiscalYearExists(connection, companyId, year) {
  ...
  if (rows.length > 0) {
    ...
    console.info(`Fiscal year ${year} already exists with status ${existingStatus}`);
    if (existingStatus === "CLOSED") {
      ...
      console.info(`Updated fiscal year ${year} status from CLOSED to OPEN`);
    }
    return;
  }
  ...
  console.info(`Created fiscal year ${year} for company ${companyId}`);
}

test("@slow Depreciation integration tests", { timeout: TEST_TIMEOUT_MS }, async () => {
  ...
  try {
    ...
    console.info(`Using companyId: ${companyId}, outletId: ${outletId}`);
    await ensureFiscalYearExists(connection, companyId, 2024);

    await runSubtest("Create fixed asset and depreciation plan", async () => {
      // Create fixed asset
      ...
      if (!createAssetResponse.ok) {
        console.info(`Failed to create fixed asset. Status: ${createAssetResponse.status}, Response: ${JSON.stringify(assetData)}`);
      }
      ...
      console.info(`Created fixed asset ${assetId} with depreciation plan ${planData.data.id}`);
    });

    await runSubtest("Run depreciation for a period posts journal batch", async () => {
      await ensureFiscalYearExists(connection, companyId, 2024);
      ...
      console.info(`Run posted journal batch ${runData.data.run.journal_batch_id} with balanced lines`);
    });

    await runSubtest("Duplicate run returns duplicate status without new journal", async () => {
      await ensureFiscalYearExists(connection, companyId, 2024);
      ...
      console.info("Duplicate run correctly returned existing journal without creating new one");
    });

    await runSubtest("Plan update blocked after posted runs", async () => {
      await ensureFiscalYearExists(connection, companyId, 2024);
      // Create asset and plan
      const assetTag = `TEST-ASSET-${randomUUID().substring(0, 8)}`;
      const createAssetResponse = await fetch(`${baseUrl}/api/accounts/fixed-assets`, {
      ...
      console.info("Plan update correctly blocked after posted runs");
    });
