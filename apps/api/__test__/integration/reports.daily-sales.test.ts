...
test(
  "@slow reports integration: daily-sales falls back to base tables when view is unavailable",
...
test(
  "@slow reports integration: daily-sales falls back to base tables when view is invalid",
  { timeout: TEST_TIMEOUT_MS, concurrent: false },
  async () => {
    const db = testContext.db;
    let txClientId = "";
