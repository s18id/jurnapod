...
function RouteScreen(props: { path: string; user: SessionUser }) {
  ...
  if (props.path === "/purchasing/orders") {
    return renderLazyPage(<PurchasingOrdersPage user={props.user} />);
  }
  ...
  return (
    <ShellProvider state={shellState}>
      <AsyncJobDrawerProvider>
        <NotificationProvider companyId={user.company_id} userId={user.id}>
          <AppLayout
            user={user}
