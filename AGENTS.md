...
**F) Deterministic tag integrity policy.**
Test/data tags generated by `makeTag(...)` MUST be treated as complete outputs. Contributors and agents MUST NOT apply post-generation slicing/truncation (for example: `makeTag(...).slice(...)` or `const t = makeTag(...); t.slice(...)`). Length constraints MUST be enforced inside the generator helper at creation time. If a truncated variant is required for a legacy field, it MUST be produced by a dedicated named helper that documents rationale and target max length.

### Protected Scripts

The following scripts have critical state-management responsibilities and MUST NOT be modified without explicit user instruction:

- `scripts/update-sprint-status.ts` — Manages `sprint-status.yaml`, the shared state file tracking all epic/story completion. This script has a dedicated validation chain (normalizeStoryInput -> resolveStoryKeys -> uniqueness gate) that replacements MUST preserve.
...
**platform**: users, roles, companies, outlets, settings, operations, audit
